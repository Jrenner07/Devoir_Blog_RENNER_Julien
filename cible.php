<?php
	...
	if(isset($_POST['pseudo']) AND isset($_POST['titre']) AND isset($_POST['message'])){	//Test d'éxistence des variables
		
		//Ajouts des variables dans le fichier liste.txt
		fputs($messages, '<div><strong>Titre : </strong>'.htmlspecialchars($_POST['titre']).'</br>');
		fputs($messages, htmlspecialchars($_POST['pseudo']).'</br>');
		...
		$ligne = fgets($messages);	//Ligne devient la ligne du fichier liste.txttxt
